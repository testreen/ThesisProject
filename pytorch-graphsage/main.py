...
def main():
    ...
    dataset_args = (config['dataset_path'], 'train', config['num_layers'])
    ...
    if config['load']:
        ...
        dataset_args = (config['dataset_path'], 'val', config['num_layers'])
        dataset = utils.get_dataset(dataset_args)
        loader = DataLoader(dataset=dataset, batch_size=config['batch_size'],
    ...
    if config['load']:
        ...
        path = os.path.join(directory, fname)
        model.load_state_dict(torch.load(path))
        dataset_args = (config['dataset_path'], 'test', config['num_layers'])
        dataset = utils.get_dataset(dataset_args)
        loader = DataLoader(dataset=dataset, batch_size=config['batch_size'],
                            shuffle=False, collate_fn=dataset.collate_wrapper)
        criterion = utils.get_criterion(config['task'])
